...
- RELATIONSHIPS: **MANDATORY**:
  1. If hasVictim is true AND the suspect is alive, they MUST have one relationship entry per deceased suspect — targetName \`The Victim\` only when there is **exactly one** isDeceased suspect; when there are **multiple** deceased suspects, use each victim's **full legal name** as targetName — plus an entry for every other **alive** suspect by name.
  2. If hasVictim is true AND the suspect is deceased (a victim), they MUST have an entry for every **other** suspect (alive or deceased) by name.
...
- **hiddenEvidence (victim, isDeceased) — TWO DISCOVERY ZONES:**
  ...
  * **environmentIncludesBody (boolean, REQUIRED when discoveryContext is "environment"):** Controls the evidence **image** asset. If **true**, the clue image may show the victim/body small or in soft focus in the background. If **false**, the image must be **environment-only** — no corpse, no body, no human remains in frame (furniture, walls, floor, objects only). The victim **examination portrait** for that clue (when the player searches that spot) uses the same rule: environment-only means a tight shot on the furniture/container — desk, open drawer, shelf — **not** a floor body scene or CSI figures.
  * For clues inside **desks, drawers, or cabinets**, **description** must say what is visibly inside or on that surface (e.g. journal, label, stain) so images can show an open drawer or tabletop hero shot that matches the narrative — not a wide room view centered on the corpse.
